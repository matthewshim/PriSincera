...
const TRANSLATIONS = {
  ko: {
    ...
    byokDesc: "Sylphio는 사용자 개인이 소유한 API Key를 직접 입력하는 안전한 방식(BYOK)을 사용합니다. 회의 종료 후 작성된 대화 텍스트 스크립트는 개발사의 중개 서버를 전혀 거치지 않고 오직 사용자의 로컬 환경과 AI 제공사의 보안 API 엔드포인트 간에만 다이렉트로 전송되며, 저장된 키는 macOS의 최고 보안 금고인 Secure Keychain(키체인)에 암호화 보존됩니다. 오디오 데이터(음성 녹음) 자체는 외부 AI 서버로 일절 전송되지 않습니다.",
    ...
    geminiFreeDesc: "비용: 100% 무료\n지원 한도: 1분당 15회 요청(15 RPM), 1일 1,500회 요청.\n실시간 자막 번역은 사용자 API Key 쿼터를 보호하기 위해 무료 Google 번역 및 온디바이스 로컬 번역 엔진으로 고정되어 작동합니다. 사용자 API Key는 세션이 종료된 후 최종 AI 회의록 요약본 및 Action Items를 생성할 때만 1회 호출되므로, 15 RPM 한도를 초과할 걱정 없이 완벽하게 요약본을 무료로 받아보실 수 있습니다.",
    ...
    openaiPriceDesc: "- 입력 토큰당 비용: $0.150 / 1M tokens (100만 토큰당 약 200원)\n- 출력 토큰당 비용: $0.600 / 1M tokens (100만 토큰당 약 800원)\n- 실시간 자막 통역은 무료/온디바이스 번역으로 진행되어 비용이 발생하지 않으며, 사용자 API Key는 세션 종료 후 최종 회의록 요약본 생성 시에만 1회 호출되어 사용됩니다. 따라서 실제 소비되는 요금은 회의 요약 시에만 발생하여 1회당 수 원 수준으로 대단히 저렴합니다.",
  ...
  en: {
    ...
    byokDesc: "Sylphio uses a secure BYOK approach where users input their own API Keys. The transcribed text script bypasses developer servers entirely and flows directly between your local device and the AI provider's secure endpoint at the end of the session. Saved keys are kept fully encrypted inside macOS's highest vault, Secure Keychain. Audio data (voice recordings) is never transmitted to third-party AI servers.",
    ...
    geminiFreeDesc: "Cost: 100% Free\nLimits: 15 Requests per Minute (RPM), 1,500 requests per day.\nReal-time subtitle translation is powered by built-in local or free engines to protect your API Key quota. The API Key is only called once at the end of the session to generate the AI minutes summary and Action Items, so you can enjoy free summaries without worrying about exceeding the 15 RPM limit.",
    ...
    openaiPriceDesc: "- Input cost: $0.150 / 1M tokens\n- Output cost: $0.600 / 1M tokens\n- Real-time subtitle translation is powered by local/free engines and incurs no cost. The API Key is only called once at the end of the session to generate the summary, costing less than a penny ($0.001 to $0.005) per session.",
  ...
  ja: {
    ...
    byokDesc: "Sylphioは、ユーザーが所有するAPIキーを直接入力する安全な方式（BYOK）を採用しています。セッション終了後に作成される対話テキストスクリプトは開発元のサーバーを一切仲介せず、ローカル端末とAIプロバイダのセキュアなエンドポイント間で直接通信されます。保存されたキーは、macOSの最高セキュリティストレージであるSecure Keychain（キーチェーン）に暗号化されて安全に保護されます。音声データ（録音ファイル）自体は外部のAIサーバーへ一切送信されません。",
    ...
    geminiFreeDesc: "料金：100% 無料\n制限：1分あたり15回のリクエスト (15 RPM), 1日あたり1,500回のリクエスト。\nリアルタイム字幕翻訳はユーザーAPIキー의 クォータを保護するため、無料のGoogle翻訳およびオンデバイスローカル翻訳エンジンに固定されて動作します。ユーザーAPIキーはセッション終了後に最終AI議事録要約およびアクションアイテムを生成する際のみ1回呼び出されるため、15 RPM制限を心配することなく無料で要約を受け取ることができます。",
    ...
    openaiPriceDesc: "- 入力料金: $0.150 / 1M tokens\n- 출력料金: $0.600 / 1M tokens\n- リアルタイム字幕翻訳は無料のローカル/Web翻訳で行われるため費用は発生せず、ユーザーAPIキーはセッション終了後の議事録要約生成時のみ1回呼び出されます。そのため、実際のトークン消費費用は要約時のみ発生し、1回あたり数円程度と極めて安価です。",

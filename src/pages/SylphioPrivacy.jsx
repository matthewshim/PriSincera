  ...
  ko: {
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          "로컬 모드 (기본 제공): 기본 번역 모드를 사용할 경우, 화면의 오디오 캡처 및 음성 인식(STT)은 Apple의 내장 프레임워크(SFSpeechRecognizer)를 통해 100% 기기 내부(On-Device)에서 오프라인으로 처리됩니다. 어떠한 오디오 데이터도 사용자의 Mac 외부로 유출되지 않습니다.",
  ...
  ja: {
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          "ローカルモード (基本提供): 基本翻訳モードを使用する場合、画面のオーディオキャプチャおよび音声認識（STT）は、Appleの内蔵フレームワーク（SFSpeechRecognizer）を通じて100%デバイス内部（On-Device）でオフライン処理されます。いかなるオーディオデータもユーザーのMac外部に流出することはありません。",

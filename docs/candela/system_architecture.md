...
| v1.2 | 2026-08-19 | AI Agent | 보완 — §5-1 해시체인 외부 앵커(증명 범위 명시)·§7 candela_* deny 실행상태·§8-1 해외주식 P4 실측 목록 추가. 결정 변경 없음 | firestore.rules, Worker |
...
> **증명 범위 — 앵커로 보강**: 해시 체인은 게시 후 소급 조작을 탐지하지만, 발행자 본인이 전체 체인을 재생성하는 자기위조까지 막지는 못한다. 따라서 위 '강한 증명'은 **외부 타임스탬프 앵커가 있을 때 성립**한다 — 일 1회 최신 `hash`를 외부에 앵커(공개 위치에 발행 해시 기록)해 발행 시점을 고정하면 '그 시점에 이 값이 존재했다'가 제3자에게 증명된다. 기존 체인 구조는 그대로 두고 앵커만 얹는다.
...
> **실행 상태(2026-08-19)**: 현행 [firestore.rules](../../firestore.rules)에는 기본 deny만 있고 `candela_*` 명시 deny는 아직 미반영이다. 컬렉션 신설 시점에 함께 등재하며 그 전까지는 기본 deny가 커버한다. 누락 방지를 위해 [roadmap §4-1](roadmap.md)의 실행 대기(E-1)에 등재했다.
...
**P4 실측 확인 목록** (해외주식 API — 결정이 아니라 채울 빈칸):

*   주문 유형 — 시장가 지원 여부, 세션 경계 주문(LOO/MOO 등) 가능성
*   호가 단위 — 종목·가격대별 tick, 소수점 호가 유무
*   정산 지연 — 체결→결제(T+N)·환전 반영 시점, 예수금 가용 타이밍
*   거래 가능 시간 — 프리/애프터 세션 접근 여부와 그때의 유동성
*   최소 주문 단위 — 소수점(fractional) 주문 지원 여부

> 목록을 미리 박아두는 이유는 실측을 빠뜨리지 않기 위함이다. 값은 P4에서 채운다.

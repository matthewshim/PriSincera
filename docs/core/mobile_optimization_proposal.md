...
## 7. 모바일 스크롤 피로도 해소 및 가로 스와이프(Horizontal Swipe) 전환 검토 [추가 제안]

모바일 가로폭 `360px` 기준 플래그십 카드 4개가 세로로 전개될 경우 총 스크롤 높이가 약 `2,800px ~ 3,200px`에 달합니다. 이는 모바일 화면 뷰포트의 약 **4~5배 분량**에 달하는 단일 섹션 스크롤 길이로서, 사용자의 피로도를 급격히 높이고 하단 문의 섹션으로의 도달률을 떨어뜨리는 주요 인지 병목을 유발합니다. 

이에 따라, 메인 인덱스(Home) 내 **Service 영역(WorkSection)을 모바일에서 가로 스와이프(Horizontal Swipe) 레이아웃으로 전환하는 설계 고도화 방안**을 적극 검토 및 제안합니다.

### 1) 가로 스와이프 전환의 이점 분석
1. **스크롤 높이의 극적인 감축**: 4개 카드 세로 합산 높이가 단 1개 카드의 높이인 약 `650px ~ 700px` 수준으로 수축되어, 세로 스크롤량이 무려 **75% 이상 격감**하는 효과를 누립니다.
2. **모던 모바일 UX 트렌드 정합**: Apple, Stripe 등 글로벌 프리미엄 테크 기업이 핵심 제품군 및 Bento 카드를 모바일에 맞추어 다이내믹 갤러리 형태로 소개하는 모바일 최적화 스킨과 완벽하게 호응합니다.
3. **정보 대조 및 인지력 제고**: 좌우 스와이프 제스처를 통해 각기 다른 고유 비주얼 테마(Amber, Indigo, Cyan, Green)를 지닌 4대 핵심 독립 브랜드들의 아이덴티티를 콤팩트한 뷰포트 내에서 빠르게 비교·각인할 수 있습니다.

### 2) 최고 수준의 사용성 보존을 위한 4대 구현 기법
스와이프 전환 시 터치 제어의 어색함이나 정보 차단을 방지하기 위해 다음과 같은 고사양 디테일 구현이 필수적입니다.
* **피크 어포던스(Peek Affordance) 설계**: 카드 너비를 `85vw` 혹은 고정 최소폭으로 제한하여, 다음 카드의 왼쪽/오른쪽 테두리가 화면 측면에 약 `12% ~ 15%` 엿보이도록 유도합니다. 이를 통해 사용자가 텍스트 가이드 없이도 가로 스와이프 가능 여부를 본능적으로 인지하게 만듭니다.
* **스크롤 스냅(Scroll Snapping)**: 드래그가 멈출 때 카드가 정확히 뷰포트 중앙에 찰칵 흡착되는 `scroll-snap-type: x mandatory` 및 `scroll-snap-align: center` 기법을 탑재하여 덜덜거리는 관성 스크롤 제어 피로도를 격파합니다.
* **120Hz 고주사율 가속**: 모바일 브라우저 렌더러가 GPU 하드웨어 가속을 적극 활용하게 유도하는 `-webkit-overflow-scrolling: touch;` 및 `will-change: transform`을 명시하여 버벅임 없는 부드러운 스와이프 질감을 보존합니다.
* **인디케이터 도트(Indicator Dots)**: 카드 덱 하단에 미니멀 글래스모피즘 도트(4개)를 렌더링하고 활성 카드 노드 인덱스 상태를 실시간 스크롤 감지 및 동기화하여 전체 항해 진행도를 명확히 파악하도록 지원합니다.

### 3) 추가 적용 검토 가능 영역 (공동 개선 검토)
* **Philosophy (Belief) 영역의 3대 컨셉 카드 (Concept Cards)**
  * *현황*: Orbit, Priority Prism, Sincera Prism 3개 카드가 모바일에서 세로로 정렬되어 약 `450px` 높이를 점유합니다.
  * *개선 검토안*: 기하학적 심볼(○, △, ▽)과 핵심 브랜드 철학을 담고 있는 카드군이므로, 가로 스와이프 덱으로 전환 시 마치 우주 은하를 공전하는 궤도를 미니멀 갤러리로 넘겨보는 듯한 고품격 인터랙티브 경험을 제공하며 전체 스크롤 길이를 추가로 세이브합니다.
* **Journey (Timeline) 영역**
  * *개선 검토안*: 로드맵 연대기 역시 가로 시간선 흐름에 맞춰 수평으로 노드를 추적하며 슬라이딩 스크롤하는 다이내믹 뷰를 제공할 경우 모바일 터치 몰입도를 한 차원 높은 수준으로 향상시킬 수 있습니다.
...
운영 및 배포의 시너지와 사용성의 비약적 동반 상승을 위해 최종 결정된 **[일괄 진행 전략(Big Bang Integration)]**을 전격 실행하여 플랫폼 전반의 모바일 가독성과 조작성을 즉각 완성할 것을 최종 합의 및 확정합니다. 더불어, 모바일 스크롤 피로도를 해결하기 위해 추가 발굴된 **[Service & Belief 가로 스와이프 통합 최적화]** 마일스톤을 차기 핵심 반응형 UX 과제로 상정하고 이를 전격 연구·반영해 나갈 것을 적극 제안합니다.

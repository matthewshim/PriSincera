...
| **API 모듈** | `pacenote-api` · `admin-api` · `study-api` | — | 라우터 단위 분리 (server.mjs가 마운트; Builder's Log 조회수는 server.mjs 인라인, 발행은 admin-api) |
...
├── 📄 admin-api.mjs            # /admin/api/* 라우터 (관리자 — Builder's Log 발행 포함)
├── 📄 study-api.mjs            # /api/study/* 어학(Language Dojo) 라우터
│
...
| `/api/pacenote/*` | `pacenote-api.mjs` | Firebase idToken | 주차 데이터·실행 궤도 CRUD, **성장 프로파일 `/profile`**(추천 개인화·루프 리포트) |
| `/admin/api/*` | `admin-api.mjs` | 관리자 | 콘텐츠 생성 트리거·구독·통계·잡 실행, **서비스 문서 수정·이력 `/docs/save`·`/docs/history`** |
| `POST /api/builderslog/:slug/view` | server.mjs 내장 | — | Builder's Log 조회수 증분 (발행·통계는 `/admin/api/builderslog/*`) |
| `/api/study/*` (별칭 `/api/pristudy/*`) | `study-api.mjs` | 조회 공개 / 진행은 Firebase idToken | 어학(Language Dojo) 콘텐츠·학습 진행 |
...
### 7-4. 캐시 정책 (server.mjs `express.static`)
...
| SPA 폴백 HTML | `no-cache` | — | 경로별 옛 번들 고착 방지 |

### 7-5. 보안 헤더 (server.mjs `helmet`)

| 헤더 | 값 | 용도 |
|------|-----|------|
| `X-Content-Type-Options` | `nosniff` | MIME 타입 스니핑 방지 |
| `X-Frame-Options` | `SAMEORIGIN` (helmet 기본값) | 외부 도메인 클릭재킹 방지 |
| `X-XSS-Protection` | 미방출 (`xssFilter: false` — Deprecated, CSP로 대체) | 구식 필터 충돌 배제 |
| `Content-Security-Policy` | helmet CSP 지시어 (server.mjs L58–76) | 외부 악성 스크립트 차단 |

...
> **[v5.4 재편]** 전수 검사 결과 구 3-Tier의 `1440px` 랜딩 핏은 실사용처가 없고(메인·Sylphio 모두 1200px 운용), `/daily`의 `1100px`는 규격 미등재 상태였습니다. **실운영 기반 4계 폭 체계**로 정본화하며, 이 4개 값 외의 컨테이너 폭은 금지합니다(신설 시 본 문서 개정 경유).

1. **정독 아티클 핏 (Reading - `900px`)**
   * **적용 범위**: `/builders-log/{slug}` (아티클 본문)
   * **목적**: 가독 임계(Line-Length)를 넘지 않는 글읽기 전용 폭.

2. **Sylphio 정독 핏 (Sylphio Reading - `1000px`)**
   * **적용 범위**: `/sylphio/guide`, `/sylphio/privacy` — 표·코드 블록 공간 확보를 위한 브랜드 제품군 상향 폭.

3. **목록·피드 핏 (List & Feed - `1100px`)**
   * **적용 범위**: `/daily` (다이제스트 목록·상세 워크스테이션)
   * **목적**: 2열 카드 그리드와 본문 가독의 균형점.

4. **대시보드·랜딩 핏 (Dashboard & Landing - `1200px`)**
   * **적용 범위**: `/pacenote`, `/builders-log`(목록), `/relearn`, 메인 Landing 콘텐츠, `/sylphio`(랜딩 섹션)
   * **목적**: 3~4열 그리드·벤토 분할과 FHD 좌우 여백의 양립. 풀블리드 비주얼(캔버스·히어로 배경)은 컨테이너 밖에서 자유.
...
**[강제 규칙 — MUST]**
1. `font-size`는 아래 **13단 허용 스케일(`--fs-*` 토큰, `index.css` 정의)** 값만 사용합니다. **px 단위·임의 값(예: 0.78rem, 0.84rem) 절대 금지.**
...
- **ERROR(차단)**: 마이크로 밴드 `[0.66rem, 1.0rem)`의 비토큰 font-size 리터럴·동일 밴드 px — ReLearn 사태와 동일 클래스의 위반.
...
현행 13단 밖에서 **관행적으로 사용 중이나 아직 규범화되지 않은 값**(check WARN 대상). 스냅 시 시각 회귀 위험이 있어 개별 결정 후 토큰 등재 또는 정리한다:
- **헤딩·디스플레이 계열**: 1.05 / 1.2 / 1.25 / 1.35 / 1.45 / 1.6 / 1.7 / 1.75 / 1.8 / 2 / 2.4 / 2.5 / 3.5(PaceNote 히어로 — §9-1 표준 3rem과 상충) / 4rem
- **장식 마이크로 라벨**: 0.55 / 0.58 / 0.6 / 0.62 / 0.64 / 0.65rem (히어로 킥커·워크 섹션 등) — `--fs-060` 신설 여부 결정
- **px 잔존**: 장식 요소 일부(9·10·18·28px 등) — rem 전환 대상
- **호버 변형 정렬**: 전수 검사(2026-07-20) 기준 사이트 전반 호버 조합 12종(~90건). §9-3 스코프 확정에 따라 **리스트형 카드류부터 단계적으로 단일 규격 정렬** (버튼·셀·미디어형은 대상 아님)
- **PaceNote 히어로 3.5rem/800**: §9-1(3rem/700)과 상충 — 정합 여부 결정 대기

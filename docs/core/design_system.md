...
- **[목록 없는 섹션의 하단 크로스 내비 — v5.20]** 섹션에 **목록 페이지가 없고** 사이드바가 교차 탐색을 겸하는 경우(뷰 중심 IA), **본문 하단 카드 스트립을 상시 배치**합니다. 사이드바는 ≈1100px 이하에서 숨겨지므로, 하단 내비가 없으면 모바일에서 다른 글로 이동할 수단이 완전히 사라집니다. 목록 페이지가 있는 섹션(Builder's Log)에서는 종전대로 하단 CTA가 선택입니다.
...
*최종 업데이트: 2026-08-31 (v5.21 §9-1 상세 히어로 정합 — Builder's Log 상세·Planner's View)*

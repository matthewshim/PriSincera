...
- **[적용 범위 — v5.9]** 본 규격은 서비스 랜딩뿐 아니라 **콘텐츠 상세 페이지의 최상위 히어로에도 적용**됩니다(예: 리런 아카이브 상세 — `.rl-hero` 클래스 재사용으로 정합). 상세 특성상 히어로 아래 보조 요소(주간 달력·브레드크럼 등) 추가는 허용하되, 3요소 구조와 토큰 수치는 불변입니다. 축소 변형 헤더를 새로 발명하지 않습니다.
...
- **적용 사례**: `BuildersLogDetail`([builders-log/ui_specification §6](../builders-log/ui_specification.md)), `SylphioApiKeyGuide`·`SylphioPrivacy`.
...
*최종 업데이트: 2026-08-12 (v5.17 디스플레이 대역 토큰 등재 + clamp endpoint 전수 토큰화·ERROR 승격)*

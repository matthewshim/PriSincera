...
│   │    · 완독 자동 전환: 채널 콘텐츠 하단이 뷰포트 60% 선을 지나면 다음 채널로
│   │    채널별 섹션(기본=트랙 단일) + '궤도로' + 구독 문맥 CTA + 아카이브 링크
...
## 2. 배움 4채널 (§계획 v1.3) — '전체' 채널은 제거(스크롤 완독 자동 순환으로 대체)
...
- **채널 전환 = 좌측 레일 2뎁스 앵커**(데스크톱) / 인라인 칩(모바일) — 상단 스티키 바 금지(콘텐츠 가림 QA 반영)

...
> **작성일:** 2026-04-29  
> **최종 업데이트:** 2026-04-30  
> **상태:** ✅ 배포 완료 — 테스트 발송 성공, Cloud Run 환경변수 설정 완료, Pipeline 빌드 트리거 구축  
...
| **구독자 DB** | **Firestore** | Cloud Run은 무상태(stateless) → 로컬 저장 불가. Firestore로 전환 완료 (GCS JSON은 폴백) |
...
### package.json (루트, web server) — ✅ nodemailer 추가 (2026-04-30)

> **중요**: Dockerfile(웹 서비스)은 루트 `package.json`만 `npm ci`하므로,
> `admin-api.mjs`에서 `pipeline/src/lib/mailer.mjs`를 import할 때
> `nodemailer`가 루트에도 있어야 합니다.
...
## 10. Cloud Run 환경변수 (✅ 설정 완료 — 2026-04-30)

### Web Service (`prisincera-web`) — ✅ 설정 완료

| 변수 | 값 | 비고 |
|------|-----|------|
| `BUTTONDOWN_API_KEY` | `fdc1a011-...` | 레거시, 향후 제거 |
| `UNSUBSCRIBE_SECRET` | `764a172e...` | HMAC 구독 해지 토큰 |
| `SMTP_USER` | `matthew.shim@prisincera.com` | Gmail SMTP 인증 |
| `SMTP_PASS` | `****` | Gmail 앱 비밀번호 |
| `SMTP_FROM_NAME` | `PriSignal` | 발신자 이름 |
| `SMTP_FROM_EMAIL` | `matthew.shim@prisincera.com` | 발신자 이메일 |

### Pipeline Jobs (`prisignal-composer`) — ✅ 설정 완료

| 변수 | 값 | 비고 |
|------|-----|------|
| `GEMINI_API_KEY` | Secret Manager 참조 | AI 스코어링 |
| `BUTTONDOWN_API_KEY` | Secret Manager 참조 | 레거시 |
| `SMTP_USER` | `matthew.shim@prisincera.com` | Gmail SMTP 인증 |
| `SMTP_PASS` | `****` | Gmail 앱 비밀번호 |
| `SMTP_FROM_NAME` | `PriSignal` | 발신자 이름 |
| `SMTP_FROM_EMAIL` | `matthew.shim@prisincera.com` | 발신자 이메일 |
...
✅ email-template.mjs        ✅ composer.mjs 수정    ✅ 테스트 발송 성공    ✅ git push + 배포
✅ mailer.mjs                ✅ buttondown.mjs 삭제  ⏸ Buttondown 종료   ✅ Pipeline 빌드 트리거
```
...
| 항목 | 상태 | 비고 |
|------|------|------|
| Cloud Run 환경변수 (Web) | ✅ 완료 | SMTP_USER/PASS/FROM_NAME/FROM_EMAIL + UNSUBSCRIBE_SECRET |
| Cloud Run 환경변수 (Composer Job) | ✅ 완료 | 동일 SMTP 변수 + GEMINI_API_KEY |
| Pipeline 빌드 트리거 생성 | ✅ 완료 | `deploy-pipeline` (pipeline/cloudbuild-pipeline.yaml) |
| 루트 package.json nodemailer 추가 | ✅ 완료 | Dockerfile 빌드 시 누락 방지 |
| Cloud Build Logs Writer 권한 | ✅ 완료 | 내부 오류 해결 |
| Firestore Timestamp 변환 버그 | ✅ 완료 | getAllSubscribers() ISO string 변환 |
| 테스트 이메일 발송 | ✅ 성공 | ksshim_kr@naver.com, matthew.shim@prisincera.com |
| 데일리 뉴스레터 샘플 발송 | ✅ 성공 | 4/30 데이터 15건, DM Pick 5건 |

### 해결된 버그

1. **nodemailer 모듈 미설치**: `pipeline/package.json`에만 있고 루트에 없어서 웹 서비스에서 import 실패 → 루트에 추가
2. **sendEmail() 호출 인터페이스 불일치**: `admin-api.mjs`에서 객체 대신 개별 인자 전달로 수정
3. **UNSUBSCRIBE_SECRET 미설정 시 크래시**: 폴백 시크릿 로직 추가
4. **개별 발송 실패 시 전체 중단**: sendToSubscribers 루프 내 try-catch 추가
5. **Firestore Timestamp → JSON 직렬화 오류**: `subscribedAt`이 `{_seconds, _nanoseconds}` 객체로 반환되어 프론트엔드에서 `Invalid Date` → `.toDate().toISOString()` 변환 추가

### 남은 과제

- [ ] Buttondown 계정 비활성화 (전환 완료 확인 후)
- [ ] SPF/DKIM DNS 레코드 확인 (스팸 방지)
- [ ] Cloud Scheduler → Composer Job 실행 스케줄 확인
- [ ] 구독자 2,000명 초과 시 SMTP Relay 또는 전용 서비스 전환 검토

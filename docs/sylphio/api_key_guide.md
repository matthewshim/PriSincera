...
* **무료 버전 (Free Tier)**: 
  ...
  - **동작 특징**: **[중요]** 현재 Sylphio의 실시간 자막 번역은 사용자 API Key 쿼터를 보호하기 위해 무료 Google 번역 및 온디바이스 로컬 번역 엔진으로 고정되어 작동합니다. 따라서 실시간 자막 통역 중에는 Gemini API가 호출되지 않으며, 사용자 API Key는 세션이 종료된 후 **최종 AI 회의록 요약본 및 Action Items를 생성할 때만 1회 호출**됩니다. 이로 인해 Free Tier 한도(15 RPM)를 초과할 걱정 없이 완벽하게 요약본을 무료로 받아보실 수 있습니다.
* **유료 버전 (Pay-as-you-go)**:
...
* **요금제 정보 (GPT-4o-mini 기준)**:
  ...
  - **동작 특징**: **[중요]** 현재 Sylphio의 실시간 자막 번역은 사용자 API Key 쿼터 및 크레딧 소모를 방지하기 위해 무료 Google 번역 및 온디바이스 로컬 번역 엔진으로 고정되어 작동합니다. 사용자 API Key는 오직 세션 종료 후 **최종 AI 회의록 요약본 및 Action Items를 생성할 때만 1회 호출**됩니다. 이에 따라 실시간 통역 세션을 몇 시간 동안 가동하더라도 실제 요약본 생성 시에만 최소한의 토큰이 소비되므로, 크레딧 비용이 사실상 거의 발생하지 않습니다.

...
| v1.5 | 2026-06-24 | AI Agent | 오빗 정책 변경: action 항목을 **항목별 독립 궤도 N개**로 주입(subtask 체크리스트 모델 폐지), **카테고리를 트랙 도메인에 매핑**. 고정 높이 flex 압축 방지(`flex-shrink:0`) | PaceNote Orbit |
...
## 12. Click-to-Orbit — v1.5 (항목별 독립 궤도)

데일리 다이제스트의 **테크 트랙 카드**(`TrackSignalFeed.jsx`)에서 시작되는 "배움 → 실행" 전환 장치이다. 사업계획서 1.1 성장 플라이휠의 Input→Action 연결.

### 12-1. 오빗 주입 정책 (v1.5 변경)
* **트리거**: 테크 트랙 카드 하단 **[＋오빗에 추가]** 버튼 → `POST /api/pacenote/add-orbit` (`{ action_challenge, domain }`, 로그인 필요).
* **결과**: `action_challenge`의 **각 항목(task)을 각각 1개의 독립 궤도(flat Task)로** 주입한다 — 항목 N개 → 궤도 N개. (이전 v1.4의 "부모 오빗 + 3 subtask 체크리스트" 모델은 **폐지**.)
* **카테고리**: 트랙 **도메인에 매핑**한다 — `ai_llm`→AI/LLM(cyan), `system_design`→System Design(indigo), `devops`→DevOps(mint), `tech_lead`→Tech Lead(gold). (이전의 "Life" 스마트 카테고리 폐지.)
* **id/멱등**: `orbit-<ac.id>-<i>`. 이미 존재하는 id는 제외하고 신규만 추가(전부 중복이면 409).
* **자동 주차 생성**: 주차 문서가 없으면 404 대신 기본 주차를 자동 생성 후 주입(`buildDefaultWeek`).
* **렌더**: 주입된 궤도는 일반 task와 동일하게 `.pacenote-task-item` 단일 카드로 렌더된다(별도 중첩 구조 없음). 고정 높이 flex 컨테이너 압축 방지를 위해 `.pacenote-task-item { flex-shrink: 0 }` 적용.
* **데스크톱**: 동일 결과를 IPC `add_orbit_from_signal`로 보장 — [data_contract_v2.md](../data_contract_v2.md) §2.2.

...
*   **🆕 [리런 통합 서비스 추진 계획서 (Product Strategy)](relearn/product_strategy.md)** (`product_strategy.md`): "제로부터 다시 배우는 러너" — 명칭 확정(리런=Re-Learn+Re-run) + 배움·실행·복기 3-stage 통합 UX + **추가형(기존 메뉴 보존) 리뉴얼** 5-Phase 로드맵. 성장 루프([growth_loop_plan](pacenote/growth_loop_plan.md)) 백엔드 연결의 표면화.

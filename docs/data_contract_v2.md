...
| **궤도 제외(soft)** | `POST /api/pacenote/exclude` | `exclude_task(taskId)` | `{ taskId: string }` | `{ success, currentPace: Task[] }` — 삭제가 아닌 `excluded: true` 플래그(데이터 보존·복원 가능). **미완료 항목만 허용**(완료 항목은 400) |
| **궤도 복원** | `POST /api/pacenote/restore` | `restore_task(taskId)` | `{ taskId: string }` | `{ success, currentPace: Task[] }` — `excluded` 해제 |
